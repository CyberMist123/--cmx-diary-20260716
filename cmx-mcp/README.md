...
MCP 请求直接连接本机 Nginx，不经过 Cloudflare：
...
`CMX_MASTODON_HOST` 很重要。Nginx 会把它传给 Mastodon，Rails HostAuthorization 才会把请求识别为当前网页域名。只配置回环 URL 而不覆盖 Host，可能得到 blocked host。
...
$env:CMX_MASTODON_HOST = "pi.ler428.xyz"
  ...
  "mcpServers": {
    "cmx-fable": {
      ...
      "env": {
        ...
        "CMX_MASTODON_HOST": "pi.ler428.xyz",
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `CMX_MASTODON_BASE_URL` | `http://127.0.0.1:8080` | 实际 TCP 连接的本机 Nginx 入口 |
| `CMX_MASTODON_HOST` | 无 | 当前 `WEB_DOMAIN`，作为 HTTP Host 发送 |
...
| `CMX_ALLOW_REMOTE_BASE_URL` | `false` | 原型默认禁止非回环连接 |
...
8. 本机 URL + WEB_DOMAIN Host 的处理是否覆盖 Rails/Nginx 边界。
