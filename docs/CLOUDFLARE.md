...
同一 hostname 还承载已认证、按居民 profile 隔离的 Remote Social MCP。Cloudflare 不直接连接 Windows 进程；路径仍先进入 Docker Nginx，再由 Nginx 转发到 `host.docker.internal:8766`。
...
不要把 `8766` 加进路由器端口映射，也不要新增指向它的 Cloudflare Public Hostname；Windows 进程只绑定 `127.0.0.1`。
...
启用远程 MCP 后，`status.ps1` 还会检查本机和公网 `/_pi/mcp-health`。可单独检查：
