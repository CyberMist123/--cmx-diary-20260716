...
- 测试分支 `fix/cmx-5000-char-limit` 的 Compose 配置和自动化测试；
- 目标 Windows 重建 `web` / `sidekiq` 后，`/api/v2/instance` 返回 `max_characters=5000`；
- 网页发布 501 字符与接近 5000 字符的动态；
- MCP 发布 501 字符与接近 5000 字符的动态；
- AI 点赞 Owner 动态后，Owner 收到 `favourite` 通知；
- 添加第一个真实 AI 居民 Token；
- DPAPI 写入/读取该 Token；
...
1. 在目标 Windows 同步 `fix/cmx-5000-char-limit`；
2. 验证 Compose 配置并重建 `web` / `sidekiq`；
3. 验证实例 API、网页 5000 字符发布和 MCP 5000 字符发布；
4. 验证 AI 点赞通知，并确认收藏不通知作者；
5. 真实 smoke 通过后合并测试分支到 `main`；
6. 再继续本地 MCP 居民接入与远程 MCP 第二阶段。
